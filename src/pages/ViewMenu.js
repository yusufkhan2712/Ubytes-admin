...
const columns = [
  ...
  { field: "productName", headerName: "Product Name", width: 50 },

  { field: "productPrice", headerName: "Product Price", width: 50 },
  { field: "branchName", headerName: "Branch Name", width: 50 },

  { field: "merchantName", headerName: "Merchant Name", width: 50 },
  {
    field: "",
    headerName: "",
    width: 260,
...
const ViewMenu = ({ history }) => {
  ...
  const getRows = async () => {
    ...
    data.forEach((element) => {
      try {
        let _ = element.data();
        _["merchantName"] = element.data().merchantName[0]?.brandName;
        _["branchName"] = element.data().branchName[0]?.branchName;
        _["id"] = element.id;
        dataarr.push(_);
      } catch {
        console.log("a");
  ...
  const deleteMerchant = async (row) => {
    setLoading(true);
    await db.collection("products").doc(row.id).delete();
    // delete logo

    if (row.productImage) {
      await storage.ref("productImage/" + row.id).delete();
    }
                    ...
                    >
                      ...
                      <TableCell>
                        ...
                        <Button
                          ...
                          startIcon={<EditIcon />}
                          onClick={() => history.push(`/edit/menu/${row.id}`)}
                        >
                          Edit
                        </Button>
                      </TableCell>
